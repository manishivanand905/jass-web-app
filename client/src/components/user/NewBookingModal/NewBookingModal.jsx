...
const NewBookingModal = () => {
  ...
  useEffect(() => {
    Object.keys(watchedValues).forEach(key => {
      if (watchedValues[key] !== undefined && watchedValues[key] !== '') {
        bookingDataRef.current[key] = watchedValues[key];
      }
    });
  }, [watchedValues.fullName, watchedValues.phone, watchedValues.email, watchedValues.vehicleModel, watchedValues.vehicleYear, watchedValues.city, watchedValues.date, watchedValues.time, watchedValues.serviceType, watchedValues.tier]);
  const steps = user ? [
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (initialData?.type && initialData?.tier) {
  ...
  }, [isOpen, initialData, setValue, reset, user]);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const selectedTier = serviceTiers[allData.serviceType]?.find(t => t.id === allData.tier);
      const priceValue = parseFloat(selectedTier?.price.replace(/[^0-9.]/g, '')) || 0;
      
      const bookingData = {
        serviceType: allData.serviceType,
        serviceTier: allData.tier,
        servicePackage: selectedTier?.name || allData.tier,
        date: allData.date,
        timeSlot: allData.time?.value,
        carBrand: allData.vehicleModel?.split(' ')[0] || 'Unknown',
        carModel: allData.vehicleModel || 'Unknown',
        carYear: allData.vehicleYear?.value || new Date().getFullYear(),
        customerName: allData.fullName,
        customerEmail: allData.email,
        customerPhone: allData.phone,
  ...
  const canProceed = () => {
    if (user) {
      switch (currentStep) {
        ...
        case 2: return watchedValues.date && watchedValues.time;
        case 3: return watchedValues.termsAccepted;
        default: return false;
      }
    }
    switch (currentStep) {
      case 1: return watchedValues.serviceType && watchedValues.tier;
      case 2: return watchedValues.fullName && watchedValues.phone && watchedValues.email && watchedValues.vehicleModel && watchedValues.vehicleYear && watchedValues.city;
  ...
  const renderStepContent = () => {
        ...
        <StepContent>
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            ...
            <p style={{ fontFamily: '"Cormorant Garamond", Georgia, serif', fontStyle: 'italic', color: 'rgba(255,255,255,0.6)', margin: '0 0 20px 0' }}>
              Confirmation sent to <span style={{ color: '#cc0000' }}>{user?.email || watchedValues.email}</span>
            </p>
    ...
    const actualStep = user ? (currentStep === 1 ? 1 : currentStep === 2 ? 3 : 4) : currentStep;
    ...
    switch (actualStep) {
      ...
      case 4:
        const selectedTierData = serviceTiers[bookingDataRef.current.serviceType || watchedValues.serviceType]?.find(t => t.id === (bookingDataRef.current.tier || watchedValues.tier));
        
        return (
          <StepContent>
            <StepEyebrow>STEP {user ? '03' : '04'}</StepEyebrow>
            <StepTitle>
            ...
            <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderLeft: '3px solid #cc0000', borderRadius: '8px', overflow: 'hidden', marginBottom: '24px' }}>
              ...
              <div style={{ padding: '20px', borderBottom: '1px solid rgba(255,255,255,0.05)', background: 'rgba(255,255,255,0.015)' }}>
                ...
                <div style={{ fontFamily: '"Barlow Condensed", Arial, sans-serif', fontSize: '0.9rem', color: 'white', lineHeight: 1.4 }}>
                  {(bookingDataRef.current.date || watchedValues.date) && new Date(bookingDataRef.current.date || watchedValues.date).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                  <br />
                  {(bookingDataRef.current.time || watchedValues.time)?.label} — Studio visit
                </div>
              </div>
              ...
              <div style={{ padding: '20px' }}>
                ...
                <div style={{ fontFamily: '"Barlow Condensed", Arial, sans-serif', fontSize: '0.9rem', color: 'white', lineHeight: 1.4 }}>
                  {user ? user.name : (bookingDataRef.current.fullName || watchedValues.fullName)} • {user ? (user.phone || 'Phone not set') : (bookingDataRef.current.phone || watchedValues.phone)}
                  <br />
                  {user ? (user.vehicleModel || 'Vehicle not set') : (bookingDataRef.current.vehicleModel || watchedValues.vehicleModel)} ({user ? '' : (bookingDataRef.current.vehicleYear?.label || watchedValues.vehicleYear?.label)}) • {user ? (user.city || 'City not set') : (bookingDataRef.current.city || watchedValues.city)}
                </div>
              </div>
            </div>
          ...
          {!showSuccess && (
            <ModalFooter>
              <FooterButton
                ...
                style={{ visibility: currentStep === 1 || (initialData?.type && initialData?.tier && currentStep === 2) ? 'hidden' : 'visible' }}
              >
                ← PREVIOUS
              </FooterButton>
